...
Scenarios 3 and 4 then explore two possible hypotheses for the observe mito-nuclear divergence, respectively: 1) divergence is kept due to female phylopatry, i.e., females exhibit lower dispersal than males leading to higher differentiation in the mtDNA than in the nuDNA; 2) divergence is kept due to selection across spaces on mtDNA haplotypes, i.e., mtDNA haplotypes exhibit higher fitness within the region of its own lineage and decreased fitness if they are brought to a different location. Note that the main conceptual differences between scenarios 3 and 4 is that divergence arises from genetic drift in scenario 3 while it arises from natural selection in scenario 4. Therefore, the primary expected pattern differences between these two scenarios are how long it takes for differentiation to arise (selection is expected to be faster, proportional to the selective pressure) and how female gene flow interacts with either drift or selection. On the latter: in scenario 3, female dispersal is set to a value lower than in all other scenarios, which increases the strength of drift. Random dispersal and drift will then dictate patterns. In scenario 4, female dispersal is kept the same as other scenarios and equal to males, so gene flow and selection will act as opposing forces: selection works to increase differentiation and gene flow works to decrease differentiation.

In comparison to the simple geography and climate models, and from a biological perspective, scenario 3 is asking: is geography pushing for differentiation but male-biased dispersal is mixing up nuDNA? Scenario 4 is asking: is geography **NOT** the factor structuring genetic differentiation, but instead **selection** is simply pushing towards mtDNA differentiation where, otherwise, it would simply observe complete admixture (even with the existing topography)?

To model *Scenario 3*  (geography + male-biased dispersal), we will decrease mean dispersal values for females, in order to simulate female phylopatry. To model *Scenario 4* (selection on mtDNA lineages, we will assigning each individual to a mtDNA lineage in the beginning of the simulation, based on their spatial location. The fitness of each lineage will be determined by its original location: fitness will be higher if individuals are within the original boundaries for its lineage than when they are outside those original boundaries. This mimics individuals in each lineage being selected positively at each of the different highlands. Male and female dispersal values are kept equal and similar to the values in all other scenarios, making it possible for individuals to move across valleys to different highlands. Under strong selection, those individuals would have low fitness, leading to divergence across the mtDNA. In this model, for selection to occur only in mtDNA and not nuDNA, you would need high dispersal values for males with some probability of male survival and reproduction in a different highland.
